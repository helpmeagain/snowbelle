{ inputs, ... }:
{
  flake.modules.homeManager.plasma =
    ...
    let
      # == Basic config
      wallpaper = "${config.home.homeDirectory}/${config.dotfiles.wallpaper}";
      panelStyle = config.dotfiles.plasma.panelStyle;
      windowsLike = panelStyle == "windowsLike";

      # == General Widgets ==
      kickoffWidget = {
        kickoff = {
          icon = if config.dotfiles.isNixos then "nix-snowflake-white" else null;
      ...
      panels =
        panelsByStyle.${panelStyle}
          or (throw "dotfiles.plasma.panelStyle inválido: \"${panelStyle}\" (use \"gnomeLike\" ou \"windowsLike\")");

      vscodeBin = if config.dotfiles.vscode.package == "vscode" then "code" else "codium";
      vscodeIcon = if config.dotfiles.vscode.package == "vscode" then "vscode" else "vscodium";
    in
    {
